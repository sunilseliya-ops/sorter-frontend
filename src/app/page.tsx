...
export default function SorterApp() {
  ...
  const run = useCallback(async () => {
    ...
    for (let i = 0; i < urls.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        predictions.forEach((p, j) => { update[i + j] = p; });
        ...
        const update: ResultMap = {};
        batch.forEach((url, j) => {
          update[i + j] = { url, label: "error", confidence: 0, package_type: "unknown", send_to_client: false, scores: {}, status: "network_error", reason: "Network error" };
        });
        processed += batch.length;
        setResults(prev => ({ ...prev, ...update }));
        setDone(processed);
  ...
  // ── Reload model (triggers backend to pull latest from GitHub) ────
  ...
  const exportXlsx = () => {
    ...
    // Sheet 2: Data (dimensions + weights)
    const dataSheet = rows.map((r, i) => {
      const L = Number(r.length_cm) || undefined;
      const B = Number(r.breadth_cm) || undefined;
      const H = Number(r.height_cm) || undefined;
      const DW = Number(r.dead_weight_kg) || undefined;
      const VW = calcVolWeight(L, B, H);
      const CW = (DW && VW) ? Math.max(DW, VW) : DW || VW || "";
      return {
        img_url:              String(r[urlCol] || ""),
        predicted_label:      results[i]?.label || "",
        package_type:         results[i]?.package_type || "",
        length_cm:            L || "",
        breadth_cm:           B || "",
        height_cm:            H || "",
        dead_weight_kg:       DW || "",
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <header style={{
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button
            ...
            title="Pull latest model from GitHub Releases"
          >
            ↻ Reload Model
          </button>
          {doneCount > 0 && (
      ...
      <main style={{ padding: "24px 28px", maxWidth: 1440, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          {rows.length > 0 && (
            <div style={{ marginTop: 12, fontSize: 12, color: "var(--text3)", padding: "8px 12px", background: "#f0f9ff", borderRadius: 6, border: "1px solid #bae6fd" }}>
              💡 For the <strong>Data</strong> tab: include columns <code style={{ fontFamily: "var(--mono)", background: "#e0f2fe", padding: "1px 5px", borderRadius: 3 }}>length_cm</code>, <code style={{ fontFamily: "var(--mono)", background: "#e0f2fe", padding: "1px 5px", borderRadius: 3 }}>breadth_cm</code>, <code style={{ fontFamily: "var(--mono)", background: "#e0f2fe", padding: "1px 5px", borderRadius: 3 }}>height_cm</code>, <code style={{ fontFamily: "var(--mono)", background: "#e0f2fe", padding: "1px 5px", borderRadius: 3 }}>dead_weight_kg</code> in your Excel. Vol. weight = L×B×H ÷ 5000.
            </div>
          )}
        </div>
            ...
            {doneCount > 0 && (
              <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
                {[
                  ...
                  ["flyer","✉ Flyer"], ["box","📦 Box"],
                  ["good","Good"], ["half_cut","Half Cut"], ["hand_issue","Hand"], ["multi_parcel","Multi"], ["access_denied","Blocked"],
                ].map(([v, l]) => (
                  <button
                    key={v}
                    onClick={() => setFilter(v)}
              ...
              {rows.length === 0 ? (
                <div style={{ padding: "80px 40px", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 13, color: "var(--text3)", lineHeight: 1.9 }}>
                    Expects a column with image URLs · Auto-detects img_url / link / image<br />
                    Handles forbidden (403) and inaccessible URLs gracefully<br />
                    Classes: good · half_cut · hand_issue · multi_parcel · flyer · access_denied
                  </div>
                </div>
              ) : (
                <div style={{ overflowX: "auto", maxHeight: "55vh", overflowY: "auto" }}>
                  <table>
                    <thead style={{ position: "sticky", top: 0, zIndex: 2, background: "#f8fafc" }}>
                      <tr>
                        {["#", "Preview", "URL", "Package", "Label", "Confidence", "Decision", "Reason"].map(h => (
                          <th key={h} style={{
                        ...
                        return (
                          <tr key={row._i} style={{ borderBottom: "1px solid #f1f5f9", background: vi % 2 === 0 ? "white" : "#fafafa" }}>
                              ...
                              {row._url ? (
                                <div style={{ position: "relative", width: 52, height: 40 }}>
                                  <img
                                    src={row._url}
                                    alt=""
                                    style={{ width: 52, height: 40, objectFit: "cover", borderRadius: 6, border: "1px solid var(--border)", display: "block" }}
                                    onError={e => {
                            ...
                            <td style={{ padding: "8px 14px", maxWidth: 200 }}>
                              <a
                                href={row._url} target="_blank" rel="noreferrer"
                                style={{ color: "#3b82f6", textDecoration: "none", fontSize: 11, fontFamily: "var(--mono)", display: "block", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 200 }}
                                title={row._url}
                              >{row._url}</a>
                            </td>
                            ...
                            <td style={{ padding: "8px 14px", width: 90 }}>
                              {res && (
                                <span style={{
                                  ...
                                  color: res.send_to_client ? "#15803d" : "#b91c1c",
                                }}>
                            ...
                            {/* Reason */}
                            <td style={{ padding: "8px 14px", maxWidth: 260, overflow: "hidden", textOverflow: "ellipsis", fontSize: 12, color: "var(--text2)" }}>
                              {res?.reason}
                            </td>
                          </tr>
                        );
          ...
          <div style={{
            ...
            {rows.length === 0 ? (
              <div style={{ padding: "80px 40px", textAlign: "center", color: "var(--text3)" }}>
                Upload an Excel file to see dimensional data here.
              </div>
            ) : (
              <>
                <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", background: "#f8fafc", fontSize: 12, color: "var(--text3)" }}>
                  Vol. weight = L × B × H ÷ 5000 &nbsp;·&nbsp; Chargeable = max(dead weight, vol. weight) &nbsp;·&nbsp; Missing dimensions shown as —
                </div>
                <div style={{ overflowX: "auto", maxHeight: "55vh", overflowY: "auto" }}>
                  <table>
                          ...
                          <tr key={i} style={{ borderBottom: "1px solid #f1f5f9", background: i % 2 === 0 ? "white" : "#fafafa" }}>
                            ...
                            <td style={{ padding: "8px 14px", maxWidth: 180 }}>
                              <a href={url} target="_blank" rel="noreferrer"
                                style={{ color: "#3b82f6", textDecoration: "none", fontSize: 11, fontFamily: "var(--mono)", display: "block", overflow: "hidden", textOverflow: "ellipsis", maxWidth: 180 }}
                                title={url}>{url}</a>
                            </td>
                            <td style={{ padding: "8px 14px" }}>
        ...
        <div style={{ marginTop: 24, textAlign: "center", fontSize: 11, color: "var(--text3)" }}>
          Sorter v2 · {API} · Classes auto-detected from model · Batches of {BATCH_SIZE}
        </div>
      </main>
